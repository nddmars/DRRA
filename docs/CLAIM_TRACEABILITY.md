...
| External secrets management (no silently-used embedded creds; file-mount provider; fail-fast) | Implemented | `backend/utils/secrets.py`, `backend/config.py`, `backend/main.py`, `tests/test_secrets.py` (DRRA-006): secrets resolve from `{NAME}_FILE` mounts (the delivery mechanism used by Vault Agent / Kubernetes / Docker secrets) with precedence over env and rotation on re-read; startup refuses to serve on development-default credentials when DEBUG is off. Native Vault/KMS SDK clients and live connection re-establishment on rotation are tracked separately |
