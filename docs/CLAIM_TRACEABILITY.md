...
| Vulnerability scanning + blocking policy | Partial | `.github/workflows/ci-cd.yml` security-scan + `.trivyignore` (DRRA finding 9): Trivy SARIF report uploaded to code scanning; Critical/High fixed-vuln **policy defined** with a documented exception file, enforcement staged (reports, not yet hard-blocking) pending dependency-CVE triage |
