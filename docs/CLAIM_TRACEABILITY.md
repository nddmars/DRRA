...
| Blocking Critical/High vuln policy for **repository dependency manifests** | Implemented | `.github/workflows/ci-cd.yml` security-scan + `.trivyignore` (DRRA finding 9): `trivy fs` blocks the build on fixed Critical/High findings in the dependency manifests (ignore-unfixed), with a documented `.trivyignore` exception process; dependency stack upgraded (tensorflow/numpy/cryptography/pyjwt/python-multipart/pillow; unused pandas removed). Scope is manifest-only — does NOT assert the deployable image/OS/runtime is vulnerability-free |
| Full-stack vuln scanning (container image, OS, frontend, Rust, pinned actions) | Future | Stronger gate: scan the built image, add a resolved lockfile + `pip-audit`, `npm audit`, `cargo audit`, and pin third-party Actions by commit SHA (DRRA finding 9 follow-up) |
