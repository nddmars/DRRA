...
| Vulnerability scanning + blocking policy | Implemented | `.github/workflows/ci-cd.yml` security-scan + `.trivyignore` (DRRA finding 9): Trivy SARIF report to code scanning; fixed Critical/High vulnerabilities **block** the build (ignore-unfixed), with a documented `.trivyignore` exception process; dependency stack upgraded (tensorflow/numpy/pandas/cryptography/pyjwt/python-multipart/pillow) to clear known CVEs |
